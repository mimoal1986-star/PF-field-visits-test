...
class VisitCalculator:
    ...
    def _calculate_metrics(self, df, calc_params=None):
        ...
        # ПРОВЕРКА КОЛОНОК (увидишь в интерфейсе)
        st.write("### 🔍 ПРОВЕРКА КОЛОНОК в _calculate_metrics")
        st.write(f"Всего колонок: {len(df.columns)}")
        st.write("Первые 10 колонок:", list(df.columns)[:10])
        
        # ВАЖНО: Создаем обязательные колонки если их нет
        required_cols = ['План на дату, шт.', 'Факт на дату, шт.', 
                         'План проекта, шт.', 'Факт проекта, шт.']
        
        for col in required_cols:
            if col not in df.columns:
                st.warning(f"⚠️ Создаю колонку: {col}")
                df[col] = 0.0
        # ПРОВЕРКА КОЛОНОК (увидишь в интерфейсе)        
