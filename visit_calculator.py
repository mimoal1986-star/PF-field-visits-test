...
class VisitCalculator:
    ...
    def _calculate_metrics(self, fact_df, calc_params=None, plan_df=None):
        ...
        # Маска для план на дату > 0
        mask_date = df['План на дату, шт.'] > 0
        
        # Маска для план проекта > 0
        mask_project = df['План проекта, шт.'] > 0
        
        # 3.1 План/Факт на дату, %
        df['План/Факт на дату,%'] = 0.0
        if mask_date.any():
            df.loc[mask_date, 'План/Факт на дату,%'] = (
                df.loc[mask_date, 'Факт на дату, шт.'] / 
                df.loc[mask_date, 'План на дату, шт.'] * 100
            ).round(1)
        
        # 3.2 План/Факт проекта, %
        ...
        # 3.5 △План/Факт на дату, шт (Факт - План)
        df['△План/Факт на дату, шт'] = (
            df['Факт на дату, шт.'] - df['План на дату, шт.']
        ).round(1)
        
        # 3.6 △План/Факт на дату, %
        df['△План/Факт на дату, %'] = 0.0
        if mask_date.any():
            df.loc[mask_date, '△План/Факт на дату, %'] = (
                (df.loc[mask_date, 'Факт на дату, шт.'] / 
                 df.loc[mask_date, 'План на дату, шт.']) - 1
            ).round(3) * 100
